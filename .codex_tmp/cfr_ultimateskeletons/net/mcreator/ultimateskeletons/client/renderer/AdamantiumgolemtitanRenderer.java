...
import net.mcreator.ultimateskeletons.Aadamantiumgolemtitanarmor;
...
import net.mcreator.ultimateskeletons.entity.layer.AdamantiumgolemtitanLayer;
import net.mcreator.ultimateskeletons.entity.model.AdamantiumgolemtitanModel;
...
public class AdamantiumgolemtitanRenderer
extends GeoEntityRenderer<AdamantiumgolemtitanEntity> {
    private static final ResourceLocation TRAIL_TEXTURE = new ResourceLocation("ultimateskeletons", "textures/particle/trail1.png");

    public AdamantiumgolemtitanRenderer(EntityRendererProvider.Context renderManager) {
        super(renderManager, (GeoModel)new AdamantiumgolemtitanModel());
        this.f_114477_ = 0.6f;
        this.addRenderLayer(new AdamantiumgolemtitanLayer((GeoRenderer<AdamantiumgolemtitanEntity>)this));
        this.addRenderLayer(new Aadamantiumgolemtitanarmor((GeoRenderer<AdamantiumgolemtitanEntity>)this));
    }

    public RenderType getRenderType(AdamantiumgolemtitanEntity animatable, ResourceLocation texture, MultiBufferSource bufferSource, float partialTick) {
        return RenderType.m_110473_((ResourceLocation)this.m_5478_((Entity)animatable));
    }

    public boolean shouldRender(AdamantiumgolemtitanEntity p_114491_, Frustum p_114492_, double p_114493_, double p_114494_, double p_114495_) {
        return true;
    }

    public void renderRecursively(PoseStack poseStack, AdamantiumgolemtitanEntity animatable, GeoBone bone, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
    ...
    public void preRender(PoseStack poseStack, AdamantiumgolemtitanEntity entity, BakedGeoModel model, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
        float scale;
    ...
    public void render(AdamantiumgolemtitanEntity entity, float entityYaw, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
        if (entity.hasTrail()) {
    ...
    private void renderTrail(AdamantiumgolemtitanEntity entityIn, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, float trailA, int packedLightIn) {
        int sampleSize = 16;
    ...
    private void renderTrail2(AdamantiumgolemtitanEntity entityIn, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, float trailA, int packedLightIn) {
        int sampleSize = 16;
    ...
    protected float getDeathMaxRotation(AdamantiumgolemtitanEntity entityLivingBaseIn) {
        return 0.0f;
    }
}
