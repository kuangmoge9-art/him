...
import net.mcreator.ultimateskeletons.Starlightuinverselayer;
import net.mcreator.ultimateskeletons.entity.StarlightskeletonEntity;
import net.mcreator.ultimateskeletons.entity.layer.StarlightskeletonLayer;
import net.mcreator.ultimateskeletons.entity.model.StarlightskeletonModel;
...
public class StarlightskeletonRenderer
extends GeoEntityRenderer<StarlightskeletonEntity> {
    private static final ResourceLocation TRAIL_TEXTURE = new ResourceLocation("ultimateskeletons", "textures/particle/trail.png");

    public StarlightskeletonRenderer(EntityRendererProvider.Context renderManager) {
        super(renderManager, (GeoModel)new StarlightskeletonModel());
        this.f_114477_ = 0.5f;
        this.addRenderLayer(new StarlightskeletonLayer((GeoRenderer<StarlightskeletonEntity>)this));
        this.addRenderLayer(new Starlightuinverselayer((GeoRenderer<StarlightskeletonEntity>)this));
    }

    public RenderType getRenderType(StarlightskeletonEntity animatable, ResourceLocation texture, MultiBufferSource bufferSource, float partialTick) {
        return RenderType.m_110473_((ResourceLocation)this.m_5478_((Entity)animatable));
    }

    public boolean shouldRender(StarlightskeletonEntity p_114491_, Frustum p_114492_, double p_114493_, double p_114494_, double p_114495_) {
        return true;
    }

    public void preRender(PoseStack poseStack, StarlightskeletonEntity entity, BakedGeoModel model, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
        float scale;
        this.scaleHeight = scale = 2.0f;
        this.scaleWidth = scale;
        if (entity.angry) {
            super.preRender(poseStack, (Entity)entity, model, bufferSource, buffer, isReRender, partialTick, packedLight, packedOverlay, 0.5f, 0.5f, 0.5f, 0.5f);
        } else {
    ...
    public void render(StarlightskeletonEntity entity, float entityYaw, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
        CoreGeoBone swordLocate1 = this.getGeoModel().getAnimationProcessor().getBone("blade2");
    ...
    private void renderTrail(StarlightskeletonEntity entityIn, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, float trailA, int packedLightIn) {
        int sampleSize = 16;
    ...
    protected float getDeathMaxRotation(StarlightskeletonEntity entityLivingBaseIn) {
        return 0.0f;
    }
}
