...
extends EntityRenderer<BallEntity> {
    private static final ResourceLocation TEXTURE = new ResourceLocation("ultimateskeletons", "textures/entities/staruni.png");

    public BallRenderer(EntityRendererProvider.Context context) {
        super(context);
    }

    public boolean shouldRender(BallEntity p_114491_, Frustum p_114492_, double p_114493_, double p_114494_, double p_114495_) {
        return true;
    }

    public void render(BallEntity entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
        float currentRadius = entityIn.getCurrentRadius();
        VertexConsumer buffer = bufferIn.m_6299_(Starrender.glowRenderType);
        ...
        for (int i = 0; i < segments; ++i) {
            ...
            for (int j = 0; j < segments * 2; ++j) {
                ...
                BallRenderer.vertex(buffer, matrix4f, matrix3f, x4, y4, z4, r, g, b, packedLightIn, 1.0f, 1.0f);
                BallRenderer.vertex(buffer, matrix4f, matrix3f, x3, y3, z3, r, g, b, packedLightIn, 1.0f, 0.0f);
                BallRenderer.vertex(buffer, matrix4f, matrix3f, x2, y2, z2, r, g, b, packedLightIn, 0.0f, 0.0f);
                BallRenderer.vertex(buffer, matrix4f, matrix3f, x1, y1, z1, r, g, b, packedLightIn, 0.0f, 1.0f);
                BallRenderer.vertex(buffer, matrix4f, matrix3f, x1, y1, z1, r, g, b, packedLightIn, 0.0f, 1.0f);
                BallRenderer.vertex(buffer, matrix4f, matrix3f, x2, y2, z2, r, g, b, packedLightIn, 0.0f, 0.0f);
                BallRenderer.vertex(buffer, matrix4f, matrix3f, x3, y3, z3, r, g, b, packedLightIn, 1.0f, 0.0f);
                BallRenderer.vertex(buffer, matrix4f, matrix3f, x4, y4, z4, r, g, b, packedLightIn, 1.0f, 1.0f);
